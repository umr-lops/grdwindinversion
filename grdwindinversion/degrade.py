...
def createLowerResL2(inputfile, resolution,overwrite=False):
    """
    ...
    if (os.path.exists(outputfile)) and overwrite is False:
        logging.info("ok %s already existing", outputfile)
        return outputfile
    if (os.path.exists(inputfile) == False):
        logging.error("%s not existing",inputfile)
        return outputfile
    ...
    for varName in ds_base.variables:
        ...
        generic_filter(ds_base[varName].values, mean, footprint=footprint, mode='constant', output=arrayOut,
                       cval=np.nan)  # Computing lower resolution
    ...
    finla_path = outputfile.replace(".nc", ".csv")
    df.to_csv(finla_path)
